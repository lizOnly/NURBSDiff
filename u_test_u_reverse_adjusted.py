...
def main(config):
    ...
    # load inp_ctrl_pts in model if model exists, otherwise generate random inp_ctrl_pts
    # check if file exists
    if os.path.exists(f'models/{object_name}_ctrpts_{ctr_pts_u}x{ctr_pts_v}_eval_{resolution_u}x{ resolution_v}_reconstruct_{out_dim_u}x{out_dim_v}.pth'):
        # model_path = config.model_path
        state_dict = torch.load(f'models/{object_name}_ctrpts_{ctr_pts_u}x{ctr_pts_v}_eval_{resolution_u}x{ resolution_v}_reconstruct_{out_dim_u}x{out_dim_v}.pth')
        # state_dict = model.state_dict()
        # for name, param in state_dict.items():
        #     print(name)
        inp_ctrl_pts = state_dict['inp_ctrl_pts']
        # knot_int_u = model['knot_int_u']
        # knot_int_v = model['knot_int_v']
        # weights = model['weights']
        inp_ctrl_pts = torch.nn.Parameter(inp_ctrl_pts, requires_grad=True).float().cuda()
        # print(inp_ctrl_pts.shape, knot_int_u.shape, knot_int_v.shape, weights.shape)
    else:
    ...
    lr_schedule1 = torch.optim.lr_scheduler.ReduceLROnPlateau(opt1, patience=10, factor=0.1, verbose=True, min_lr=1e-4, 
        ...
        def closure():
            ...
            if ignore_uv:
                ...
                if loss_type == 'chamfer':
                    ...
                    # + 1 * hausdorff_distance(out, tgt)  
                    # + close_loss_column
                    log_value('chamfer_distance', chamfer_distance(out, tgt), i)
                    log_value('laplacian_loss', lap, i)
    ...
    #     pbar.set_description("Loss %s: %s" % (i+1, loss.item()))
    torch.save({'inp_ctrl_pts': inp_ctrl_pts, 'weights': weights}, f'models/{object_name}_ctrpts_{ctr_pts_u}x{ctr_pts_v}_eval_{resolution_u}x{ resolution_v}_reconstruct_{out_dim_u}x{out_dim_v}.pth')
